...
{
	UCLAGL_v2g output;
	output.pos3D = v.vertex;//passage des vertex en 3D
...
void UCLAGL_geom(triangle UCLAGL_v2g p[3], inout TriangleStream<UCLAGL_g2f> triStream)
{
	...
	/////////////////////////3D edge vector (gestion edge longue)
	float3 vec0 = p[2].pos3D - p[1].pos3D;
	float3 vec1 = p[2].pos3D - p[0].pos3D;
	float3 vec2 = p[1].pos3D - p[0].pos3D;
	...
	pIn.uv = p[0].uv;
	/////////////////////test distance
	if (length(vec0) > length(vec1) && length(vec0) > length(vec2)) {
		pIn.dist = float3(1000000, 0, 0);//solution à court terme
	}
	else {
	...
	pIn.uv = p[1].uv;
	/////////////////////test distance
	if (length(vec1) > length(vec0) && length(vec1) > length(vec2)) {
		pIn.dist = float3(0, 1000000, 0);//solution à court terme
	}
	else {
	...
	pIn.uv = p[2].uv;
	/////////////////////test distance
	if (length(vec2) > length(vec0) && length(vec2) > length(vec1)) {
		pIn.dist = float3(0, 0, 1000000);//solution à court terme
	}
	else{
